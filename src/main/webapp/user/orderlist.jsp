    ...
    <table>
        ...
        <c:forEach items="${orders}" var="order" varStatus="index">
            <tr>
                ...
                <td><input type="button" value="删除" onclick="javascript:window.location='<%=basePath%>/user/deleteOrderServlet?id=${order.id}';">
                    <input type="button" value="明细"
                           onclick="javascript:window.location='<%=basePath%>/user/showOrderDetailServlet?orderFormId=${order.id}';">
                    <form action="<%=basePath%>/user/payServlet?id=${order.id}" method="post">
